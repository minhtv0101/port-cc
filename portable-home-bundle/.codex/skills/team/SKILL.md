...
Agents with `memory: project` retain learnings across team sessions. Memory persists in `$HOME/.claude/agent-memory/<name>/` (gitignored). Useful for:
...
See `$HOME/.claude/rules/team-coordination-rules.md` for teammate behavior rules.
