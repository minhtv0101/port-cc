...
1. Embed design tokens inline (for `file://` compatibility)
   ```bash
   node $HOME/.claude/skills/design-system/scripts/embed-tokens.cjs --minimal
   ```
...
```bash
python $HOME/.claude/skills/design-system/scripts/search-slides.py "[goal]" -d strategy
```

### 3. Contextual Search Per Slide
```bash
python $HOME/.claude/skills/design-system/scripts/search-slides.py "[slide goal]" \
...
**Data location:** `$HOME/.claude/skills/design-system/data/`
