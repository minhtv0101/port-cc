...
**Config location:** `$HOME/.claude/secrets/google_client_secret.json` (project) or `~/.claude/secrets/` (global)
...
8. Place in $HOME/.claude/secrets/ (project scope) or ~/.claude/secrets/ (global)
```
