...
```bash
cd $HOME/.claude/skills/chrome-devtools/scripts && npm install
```
...
node screenshot.js --url https://example.com --full-page true \
  --output $HOME/.claude/chrome-devtools/screenshots/seo-audit.png
```
...
node screenshot.js --url https://example.com \
  --output $HOME/.claude/chrome-devtools/screenshots/atf.png
```
...
```bash
node evaluate.js --script "page.setViewport({width: 375, height: 812, isMobile: true})"
node screenshot.js --output $HOME/.claude/chrome-devtools/screenshots/mobile.png
```
...
OUT="$HOME/.claude/chrome-devtools/seo-audits/$SESSION"
mkdir -p "$OUT"
