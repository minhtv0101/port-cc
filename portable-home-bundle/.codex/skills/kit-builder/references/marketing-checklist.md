...
1. **New Skill**
   ```bash
   python $HOME/.claude/skills/kit-builder/scripts/init_component.py skill {name}
   ```

2. **New Command**
   ```bash
   python $HOME/.claude/skills/kit-builder/scripts/init_component.py command {name}
   ```

3. **Add to Agent**
   - Edit `$HOME/.claude/agents/{agent}.md`
